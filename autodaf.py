...
# PENTING: Gunakan di dalam OS Ubuntu Termux yang sudah terinstall chromium & chromedriver
try:
    ...
    print("❌ Library belum lengkap! Jalankan: pip install selenium selenium-wire requests")
    sys.exit(1)
...
class FBBotSeleniumCLI:
    ...
    def request_interceptor(self, request):
        # Memblokir gambar & aset berat agar Termux tidak ngelag (Sama seperti kodemu)
    ...
    def init_browser(self):
        ...
        options.add_argument("--headless=new") # Wajib untuk Termux CLI
        options.add_argument("--no-sandbox")
        options.add_argument("--disable-dev-shm-usage")
        
        # 🔥 MEMAKSA CHROME MENGGUNAKAN BAHASA INGGRIS 🔥
        ...
        # Mulai browser dengan PROXY KOSONG (Proxy OFF)
        try:
            self.driver = webdriver.Chrome(options=options)
            self.driver.proxy = {} 
            self.driver.request_interceptor = self.request_interceptor
            self.log("✅ Browser berjalan (Mode English, Proxy OFF).")
    ...
    def auto_register_flow(self):
        ...
        # 1. FORM NAMA (Sama persis dengan JS aslimu)
        js_name = """
        ...
        # 2. HALAMAN DOB & USIA
        js_dob = "return 'FOUND_DOB';" # Bypass logika cek aslimu agar cepat
        self.execute_js_with_wait(js_dob, timeout=10, step_name="Halaman DOB")
        ...
        # 5. PILIH GENDER (Mencari teks Male/Female/Laki/Perempuan persis kodemu)
        ...
        if self.use_proxy and self.proxy_str:
            ...
            time.sleep(2) # Beri waktu selenium-wire apply proxy

        # 7. KLIK DAFTAR (Persis script aslimu)
        js_click_daftar = """
            var cand=[];
        ...
        # MATIKAN PROXY (Opsional, agar pas nunggu OTP tidak lemot)
        self.driver.proxy = {} 
...
def main():
    print("====================================================")
    print(" 🤖 BOT FB DAFTAR (SELENIUM CLI - ENGLISH - DYNAMIC PROXY) ")
    print("====================================================")
